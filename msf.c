...
#define MIN_MASK        __extension__ 0b0000111111111111
...
ISR(TIMER1_OVF_vect, ISR_BLOCK)
{
	...
	if ((history & MIN_MASK) == MIN_PATT) {
		...
		set_msf_bit(msf_bit_a, sec,
			    ((history & __extension__ 0b0000100000000000)
			     > 0));
		set_msf_bit(msf_bit_b, sec,
			    ((history & __extension__ 0b0000010000000000)
			     > 0));
		fprintf(stderr, "\t%02d ", sec);
		fprintf(stderr, " %d %d %d %d", e_hi, e_lo, e_hi + e_lo,
			tick);
		if (!track) {
		    fprintf(stderr, "\n");
		}
...
#ifdef DEBUG
    fprintf(stderr, "\nYear: 20%02d ",
	    (time->ten_year * 10) + time->one_year);
#endif
...
#ifdef DEBUG
    fprintf(stderr, "\nMonth: %02d",
	    (time->ten_month * 10) + time->one_month);
#endif
...
#ifdef DEBUG
    fprintf(stderr, "\nHOUR: %02d:",
	    (time->ten_hour * 10) + time->one_hour);
#endif
...
#ifdef DEBUG
    fprintf(stderr, "\nMIN: %02d ",
	    (time->ten_minute * 10) + time->one_minute);
#endif
    if (((getparity(39, 51) + get_msf_bit(msf_bit_b, 57)) & 0x01) == 0) {
	parity_error_count++;
#ifdef DEBUG
