...
void ds3231_writetime(time_t time)
{
	...
	ds3231_write(0x01, (Uint8ToBcd(ds3231.tm_min) & 0x7f)); // Minutes
	ds3231_write(0x02, (Uint8ToBcd(ds3231.tm_hour) & 0x3f)); // Hours - Force 24 hour
	ds3231_write(0x03, (Uint8ToBcd(ds3231.tm_wday-1) & 0x07)); // Hours - Force 24 hour
	ds3231_write(0x04, (Uint8ToBcd(ds3231.tm_mday) & 0x3f)); // Day of month
	ds3231_write(0x05, (Uint8ToBcd(ds3231.tm_mday) & 0x3f) | ( ); // Day of month
}
struct tm  ds3231_tm;
time_t ds3231_readtime(void)
{
	...
	ds3231_tm.tm_year = (((val & 0x80) >> 7) * 100) + BcdToUint8(ds3231_read(0x06));
